#needed to change the catergory of a couple species from 
#"Other Bacteria" to "Eurkayote"; creating new files to be used 
#in subsequent scripts and testing them here first

#called recent data participants 26 to 64
nwdata <- read.delim(file.path("a1B2_26_64.txt"))

#called older data participants 01 to 29 (26 in newer data)
olddata <- read.delim(file.path ("a1B2data_part1.txt"))
...
#write to file
write.table(data2, "anew1B2.csv", sep = ",", row.names = FALSE, quote = FALSE)
write.table(nwdata, "a1B2_26_64.csv", sep = ",", row.names = FALSE, quote = FALSE)
write.table(olddata, "a1_291B2.csv", sep = ",", row.names = FALSE, quote = FALSE)
...
write.table(dataall, "acomplete1B2data.csv", sep = ",", row.names = FALSE, quote = FALSE)

#assigned bacterial groups and called back
data <- read.csv(file.path("acomplete1B2data.csv"))
#grouped based on bacteria labels
cdata <- ddply(data,"Group",numcolwise(sum))

#write this table to file
write.table(cdata, "a1B2data_groups.csv", sep = ",", row.names = FALSE, quote = FALSE)

#everything looks good; will use "a1B2data_groups.csv" 
#in subsequent R scripts