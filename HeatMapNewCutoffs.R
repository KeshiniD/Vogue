...
hr1<-hclust(hiv.dist, "complete")
hr2<-hclust(hiv.dist, "single")
hr3<-hclust(hiv.dist, "aver")#use this one
par(mfrow=c(1,3))
plot(hr1, hang=-1)
plot(hr2, hang=-1)
plot(hr3, hang=-1)
...
hiv.dist2<-vegdist(t(hiv.data))#makes distance matrix on transposed data
...
plot(annHeatmap2(as.matrix(hiv.data),col=rgb.palette(11), legend=3, breaks=10, dendrogram=list(Row=list(dendro=as.dendrogram(hr3)), Col=list(dendro=as.dendrogram(hc3))), cluster=list(Row=list(cuth=0.85),col=clus.col2), labels=list(Col=list(nrow=20.3))), widths=c(1,7.5), heights=c(1,1,9.5))
